...
3. Le message est envoyé via un **transport** à une file d'attente (**Adapters**), aussi appelé **système de queue** (Redis, RabbitMQ, Doctrine, …),

5. Un **worker** va chercher en temps réel les messages depuis le système de queue via le transport,

7. Il re-dispatche le message dans le bus,

9. Le message est consommé par un **handler**.
